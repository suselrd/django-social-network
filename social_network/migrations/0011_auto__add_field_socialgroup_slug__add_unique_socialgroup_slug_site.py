...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'SocialGroup.slug'
        db.add_column(u'social_network_socialgroup', 'slug',
                      self.gf('django.db.models.fields.SlugField')(default='-', max_length=255),
                      keep_default=False)

        # filling group slugs
        for group in orm.SocialGroup.objects.all():
            group.slug = slugify(group.name)
            good_slug = False
            counter = 1
            while not good_slug:
                if orm.SocialGroup.objects.filter(slug=group.slug, site=group.site).exists():
                    group.slug = group.slug[:-1] + '%s' % counter
                    counter += 1
                else:
                    good_slug = True
            group.save()
    ...
    def backwards(self, orm):
        # Deleting field 'SocialGroup.slug'
        db.delete_column(u'social_network_socialgroup', 'slug')
    ...
    models = {
        ...
        u'social_network.groupfeeditem': {
            'Meta': {'ordering': "('-event__date',)", 'object_name': 'GroupFeedItem'},
        ...
        u'social_network.socialgroup': {
            'Meta': {'unique_together': "(('slug', 'site'),)", 'object_name': 'SocialGroup'},
